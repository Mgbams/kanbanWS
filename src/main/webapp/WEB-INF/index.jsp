...
<body>
	<div class="container">
				...
				<table
					...
					<tbody>
						<c:forEach var="tache" items="${taches}">
							<tr>
								<c:forEach var="i" begin="1" end="${tache.colonneActuelle.id - 1}" step="1">
									<td width="25%" class="drop_${i}"></td>
								</c:forEach>
										<td width="25%" class="drop_${tache.colonneActuelle.id}"><a href="#" type="button"
											class="btn detail_tache link_drag" data-idTache="${tache.id}"
											style="color: white; background-color:#${tache.typeTache.couleur}"
											data-toggle="modal">${tache.intitule}<br>${tache.developpeur.prenom}</a></td>
								<c:forEach var="i" begin="${tache.colonneActuelle.id + 1}" end="4" step="1">
									<td width="25%" class="drop_${i}"></td>
								</c:forEach>
							</tr>
						</c:forEach>
		...
		<a href="taches">Liste des tâches</a>
		<br>
